SELECT
    ...
    a.year,
    a.week,
    sum(excess_adj_mortality_capita) * pop_25_44_all / 100000 AS excess_ttl
FROM
    (
        SELECT
            a.state,
        ...
        FROM
            (
                SELECT
                    ...
                    year,
                    week,
                    adj_mortality_25_44 AS adj_mortality,
                    pop_25_44 AS pop
                FROM
            ...
            JOIN (
                SELECT
                    `state`,
                    week,
                    AVG(adj_mortality_25_44) AS adj_mortality_baseline
                FROM
                    deaths.adj_mortality_week
                WHERE
    ...
    JOIN (
        SELECT
            year,
            week,
            sum(pop_25_44) AS pop_25_44_all
        FROM
            deaths.adj_mortality_week
        WHERE
            year IN (2020, 2021, 2022)
