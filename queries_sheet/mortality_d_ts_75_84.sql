SELECT
    ...
    a.year,
    a.week,
    sum(excess_adj_mortality_capita) * pop_75_84_all / 100000 AS excess_ttl
FROM
    (
        SELECT
            a.state,
        ...
        FROM
            (
                SELECT
                    ...
                    year,
                    week,
                    adj_mortality_75_84 AS adj_mortality,
                    pop_75_84 AS pop
                FROM
            ...
            JOIN (
                SELECT
                    `state`,
                    week,
                    AVG(adj_mortality_75_84) AS adj_mortality_baseline
                FROM
                    deaths.adj_mortality_week
                WHERE
    ...
    JOIN (
        SELECT
            year,
            week,
            sum(pop_75_84) AS pop_75_84_all
        FROM
            deaths.adj_mortality_week
        WHERE
            year IN (2020, 2021, 2022)
